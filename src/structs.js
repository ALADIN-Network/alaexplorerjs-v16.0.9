...
module.exports = (config = {}, extendedSchema) => {
  ...
  // If alanode does not have an ABI setup for a certain action.type, it will throw
...
const PublicKeyEcc = (validation) => {
  return {
    ...
    toObject (value) {
      if (validation.defaults && value == null) {
        const keyPrefix = validation.keyPrefix ? validation.keyPrefix : 'ALA'
...
const abiOverride = structLookup => ({
  ...
  'setabi.abi.appendByteBuffer': ({fields, object, b}) => {
    const ser = structLookup('abi_def', 'alaio')
