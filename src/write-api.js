...
function writeApiGen(Network, network, structs, config, abis) {
  ...
  /**
    ...
    @example ala.contract('mycontract').then(mycontract => mycontract.myaction(...))
  */
  merge.contract = (...args) => {
  ...
  */
  const genTransaction = (structs, merge) => async function(...args) {
    ...
    if(isContractArray) {
      ...
      // Ala contract operations are cached (efficient and offline transactions)
      const cachedCode = new Set(['alaio', 'alaio.token', 'alaio.null'])
  ...
  function genMethod(type, definition, transactionArg, account = 'alaio.token', name = type) {
  ...
  async function transaction(arg, options, callback) {
    ...
    for(const txField of [
      ...
      if(arg[txField] !== undefined) {
        // ala.transaction('alaio', alaio => { alaio.myaction(..) }, {delay_sec: 369})
        // ala.transaction({delay_sec: 369, actions: [...]})
        rawTx[txField] = arg[txField]
      } else if(options[txField] !== undefined) {
        // ala.transaction(tr => {tr.transfer(...)}, {delay_sec: 369})
        rawTx[txField] = options[txField]
      }
    }

    // alaexplorerjs calcualted headers
